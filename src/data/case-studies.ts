...
export const unifiedContextCaseStudy: CaseStudyData = {
  ...
  tagline: "An 8-agent cross-platform AI assistant with stateful SQLite memory sync across Email, WhatsApp, and Instagram.",
  tags: ["Product Case Study", "Multi-Agent System", "2025", "Solo Project"],
  stats: [
    { value: "8", label: "Specialized Agents" },
    { value: "3", label: "Unified Platforms" },
    { value: "<1.2s", label: "Response Latency" },
    { value: "100%", label: "SQLite Context Sync" },
  ],
  accentColor: "#14B8A6",
  overview: {
    description:
      "A cross-platform orchestration layer for agentic assistants, designed around a shared memory substrate and coordinated decision pipeline. It enables seamless conversation status across messaging surfaces and explicit modular responsibilities.",
    fullDescription:
      "Spans ingestion, summarisation, intent classification, context-aware response, visual memory tagging, cognitive scoring, synthetic data generation, and RL decision-making — unified across platforms via shared SQLite memory.",
    role: "Lead Architect & AI Systems Engineer",
    timeline: "4 Weeks (Q1 2025)",
    stack: ["Multi-Agent", "Flask", "SQLite", "HuggingFace", "RL", "Streamlit"],
  },
  problem: {
    heading: "Context Fragmentation Across Platforms",
    quote: "People interact with products through multiple threads — email, WhatsApp, social channels. If your AI assistant treats each message like a brand new stranger, the conversation feels mechanical and frustrating.",
    cards: [
      {
        title: "User Friction",
        desc: "Having to repeat details to the assistant because you moved from WhatsApp to Email. Memory fragmentation kills assistant utility.",
      },
      {
        title: "Integration Overhead",
        desc: "Connecting API webhooks across WhatsApp, Instagram, and Gmail usually results in separated logic micro-silos with sync issues.",
      },
      {
        title: "Coordinating Agents",
        desc: "Multiple agents must collaborate on intent, tone, summarization, and actions without executing competing, expensive LLM calls.",
      },
    ],
    whyBuilt:
      "I noticed I was communicating with clients on WhatsApp but tracking project scopes in email. When I built an assistant to draft response quotes, it kept offering outdated estimates because it missed details on WhatsApp. I realized the secret to agentic power isn't stronger models — it's unified memory.",
  },
  research: {
    heading: "Orchestration & Synchronization Insights",
    approach:
      "I mapped my own communications over 2 weeks and tested modular prompt-routing scripts on real historical client transcripts to analyze agent intent reliability.",
    insights: [
      {
        title: "Context Shifts",
        desc: "42% of project negotiations start on social channels (Instagram/WhatsApp) but shift to Email for final execution details.",
      },
      {
        title: "Low Latency Limit",
        desc: "Messaging channels have a strict expectation of conversational speed. Above 1.5 seconds, users experience chat drop-off.",
      },
      {
        title: "Intent Bottleneck",
        desc: "Routing intents via broad system instructions in a single LLM call has a 20% failure rate. Specialized agents excel at narrow tasks.",
      },
    ],
    personas: [
      {
        name: "Alex, 29 (The Digital Agency Owner)",
        role: "Manages 10+ client threads across Instagram, WhatsApp, and Email.",
        pain: "Loses track of task agreements made during casual DM discussions.",
        need: "Unified memory assistant that tracks project context across all channels.",
        quote: "I need my assistant to remember what a client agreed to on WhatsApp when I draft the final scope email.",
        tag: "Agency Focus",
      },
    ],
  },
  solution: {
    heading: "Strategic Coordination Choices",
    decisions: [
      {
        title: "Decision 1: SQLite Shared DB vs. Vector DB Index",
        choice: "Central SQLite Shared relational memory structure",
        rationale:
          "Stateful relational schemas allowed robust transactional intent consistency across WhatsApp, Instagram, and Email without indexing delays. It ensures instant synchronicity.",
        tradeoff: "No dense vector querying natively without extension. Accepted. SQLite manages the metadata state, while FAISS holds the semantic embeds.",
      },
      {
        title: "Decision 2: Regex/TF-IDF Intent Routing Agent vs. LLM Router",
        choice: "Dedicated lightweight routing logic micro-agent",
        rationale:
          "Instead of sending the user's raw message to a costly LLM router for classification, a dedicated script routes requests under 50ms, saving API costs and reducing total loop latency.",
        tradeoff: "Brittle on highly ambiguous input phrasing. Accepted. Fallback checks direct queries to LLMs when confidence scores drop.",
      },
    ],
  },
  architecture: {
    heading: "Unified Context Workflow",
    steps: [
      {
        title: "Multi-Platform Webhooks",
        subtitle: "WhatsApp, IG, Email",
        desc: "Ingests real-time message streams, normalizing varying API payload schemas into a single standardized internal state.",
      },
      {
        title: "Intent Classifier Agent",
        subtitle: "Fast Routing",
        desc: "Parses intent, mapping incoming signals to specialized micro-agents for drafting, summaries, or database queries.",
      },
      {
        title: "SQLite Memory Layer",
        subtitle: "Shared State",
        desc: "Maintains temporal, stateful relational threads so all 8 agents draw from the same absolute user context.",
      },
      {
        title: "RL Decision Loop",
        subtitle: "Reinforced Tone",
        desc: "Checks drafted agent responses against historic positive-reinforcement patterns to align language styles.",
      },
      {
        title: "Unified Response",
        subtitle: "Outbound Dispatch",
        desc: "Dispatches the optimal synthesized agent output directly to the originating webhook surface in under 1.2s.",
      },
    ],
  },
  tradeoffs: [
    {
      ...
      chose: "SQLite Shared DB",
      rejected: "Vector DB Index Only",
      why: "Provides instantaneous state synchronicity and strict ACID transactions.",
    },
    {
      decision: "Intent Routing",
      chose: "Specialized Script Agent",
      rejected: "LLM Router API",
      why: "Sub-50ms latency gain and zero API runtime cost.",
    },
    {
      decision: "Agent Coordination",
      chose: "Event-Driven Queue",
      rejected: "Sequential Loops",
      why: "Prevents blocking operations, allowing agents to execute concurrently.",
    },
  ],
  metrics: [
    { value: "8", label: "Microservice Agents" },
    { value: "3", label: "Integrated Platforms" },
    { value: "95%+", label: "Shared Intent Accuracy" },
    { value: "<1.2s", label: "Multi-agent loop latency" },
  ],
  reflection: {
    heading: "Future V2 Milestones",
    future: [
      {
        title: "Local LLM Execution",
        desc: "Move agent intent scoring completely to local Llama-3-8B containers to eliminate external API dependency.",
        priority: "High",
      },
      {
        title: "Dynamic Intent Expansion",
        desc: "Support self-learning intent categories by grouping unclassified requests automatically via unsupervised clustering.",
        priority: "Medium",
      },
    ],
    closing:
      "Building this system proved that agent coordination requires strict state ownership. A unified shared memory is the foundation of believable conversational intelligence.",
  },
};
